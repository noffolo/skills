...
- Each skill declares its own permissions following the principle of least privilege.
- `exec` covers the `meitu` CLI only. This root skill does not have `node` permission.
- The `node` permission in `meitu-tools` is restricted to the internal runner script (`meitu-tools/scripts/run_command.js`).
