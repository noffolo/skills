...
_version() { echo "sms v2.0.0"; }

_help() {
    echo "Sms v2.0.0 — content toolkit"
    echo ""
    echo "Usage: sms <command> [args]"
    echo ""
    echo "Commands:"
    echo "  draft              Draft"
    echo "  edit               Edit"
...
_search() {
    local term="${1:?Usage: sms search <term>}"
...
case "${1:-help}" in
    draft)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/draft.log" 2>/dev/null || echo "  No entries yet. Use: sms draft <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/draft.log"
            local total=$(wc -l < "$DATA_DIR/draft.log")
            echo "  [Sms] draft: $input"
    ...
    edit)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/edit.log" 2>/dev/null || echo "  No entries yet. Use: sms edit <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
    ...
    optimize)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/optimize.log" 2>/dev/null || echo "  No entries yet. Use: sms optimize <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/optimize.log"
            local total=$(wc -l < "$DATA_DIR/optimize.log")
            echo "  [Sms] optimize: $input"
    ...
    schedule)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/schedule.log" 2>/dev/null || echo "  No entries yet. Use: sms schedule <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/schedule.log"
            local total=$(wc -l < "$DATA_DIR/schedule.log")
            echo "  [Sms] schedule: $input"
    ...
    hashtags)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/hashtags.log" 2>/dev/null || echo "  No entries yet. Use: sms hashtags <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/hashtags.log"
            local total=$(wc -l < "$DATA_DIR/hashtags.log")
            echo "  [Sms] hashtags: $input"
    ...
    hooks)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/hooks.log" 2>/dev/null || echo "  No entries yet. Use: sms hooks <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/hooks.log"
            local total=$(wc -l < "$DATA_DIR/hooks.log")
            echo "  [Sms] hooks: $input"
    ...
    cta)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/cta.log" 2>/dev/null || echo "  No entries yet. Use: sms cta <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
    ...
    rewrite)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/rewrite.log" 2>/dev/null || echo "  No entries yet. Use: sms rewrite <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/rewrite.log"
            local total=$(wc -l < "$DATA_DIR/rewrite.log")
            echo "  [Sms] rewrite: $input"
    ...
    translate)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/translate.log" 2>/dev/null || echo "  No entries yet. Use: sms translate <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/translate.log"
            local total=$(wc -l < "$DATA_DIR/translate.log")
            echo "  [Sms] translate: $input"
    ...
    tone)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/tone.log" 2>/dev/null || echo "  No entries yet. Use: sms tone <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
    ...
    headline)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/headline.log" 2>/dev/null || echo "  No entries yet. Use: sms headline <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/headline.log"
            local total=$(wc -l < "$DATA_DIR/headline.log")
            echo "  [Sms] headline: $input"
    ...
    outline)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/outline.log" 2>/dev/null || echo "  No entries yet. Use: sms outline <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/outline.log"
            local total=$(wc -l < "$DATA_DIR/outline.log")
            echo "  [Sms] outline: $input"
    ...
    *)
        ...
        echo "Run 'sms help' for available commands."
        exit 1
        ;;
esac