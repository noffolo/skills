#!/usr/bin/env bash
# gear -- Gear ratio and transmission calculator
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${GEAR_DIR:-$HOME/.gear}"
...
show_help() {
    cat << EOF
gear v$VERSION -- Gear ratio and transmission calculator

Usage: gear <command> [args]
...
cmd_add() {
    local value="${1:?Usage: gear add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: gear search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: gear remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "gear v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: gear help"; exit 1 ;;
esac
