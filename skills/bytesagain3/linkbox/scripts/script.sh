...
_version() { echo "linkbox v2.0.0"; }

_help() {
    echo "Linkbox v2.0.0 — productivity toolkit"
    echo ""
    echo "Usage: linkbox <command> [args]"
    echo ""
    echo "Commands:"
    echo "  add                Add"
    echo "  plan               Plan"
...
_search() {
    local term="${1:?Usage: linkbox search <term>}"
...
case "${1:-help}" in
    add)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/add.log" 2>/dev/null || echo "  No entries yet. Use: linkbox add <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/add.log"
            local total=$(wc -l < "$DATA_DIR/add.log")
            echo "  [Linkbox] add: $input"
    ...
    plan)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/plan.log" 2>/dev/null || echo "  No entries yet. Use: linkbox plan <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/plan.log"
            local total=$(wc -l < "$DATA_DIR/plan.log")
            echo "  [Linkbox] plan: $input"
    ...
    track)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/track.log" 2>/dev/null || echo "  No entries yet. Use: linkbox track <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/track.log"
            local total=$(wc -l < "$DATA_DIR/track.log")
            echo "  [Linkbox] track: $input"
    ...
    review)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/review.log" 2>/dev/null || echo "  No entries yet. Use: linkbox review <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/review.log"
            local total=$(wc -l < "$DATA_DIR/review.log")
            echo "  [Linkbox] review: $input"
    ...
    streak)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/streak.log" 2>/dev/null || echo "  No entries yet. Use: linkbox streak <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/streak.log"
            local total=$(wc -l < "$DATA_DIR/streak.log")
            echo "  [Linkbox] streak: $input"
    ...
    remind)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/remind.log" 2>/dev/null || echo "  No entries yet. Use: linkbox remind <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/remind.log"
            local total=$(wc -l < "$DATA_DIR/remind.log")
            echo "  [Linkbox] remind: $input"
    ...
    prioritize)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/prioritize.log" 2>/dev/null || echo "  No entries yet. Use: linkbox prioritize <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/prioritize.log"
            local total=$(wc -l < "$DATA_DIR/prioritize.log")
            echo "  [Linkbox] prioritize: $input"
    ...
    archive)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/archive.log" 2>/dev/null || echo "  No entries yet. Use: linkbox archive <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/archive.log"
            local total=$(wc -l < "$DATA_DIR/archive.log")
            echo "  [Linkbox] archive: $input"
    ...
    tag)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/tag.log" 2>/dev/null || echo "  No entries yet. Use: linkbox tag <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/tag.log"
            local total=$(wc -l < "$DATA_DIR/tag.log")
            echo "  [Linkbox] tag: $input"
    ...
    timeline)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/timeline.log" 2>/dev/null || echo "  No entries yet. Use: linkbox timeline <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/timeline.log"
            local total=$(wc -l < "$DATA_DIR/timeline.log")
            echo "  [Linkbox] timeline: $input"
    ...
    report)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/report.log" 2>/dev/null || echo "  No entries yet. Use: linkbox report <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/report.log"
            local total=$(wc -l < "$DATA_DIR/report.log")
            echo "  [Linkbox] report: $input"
    ...
    weekly-review)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/weekly-review.log" 2>/dev/null || echo "  No entries yet. Use: linkbox weekly-review <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/weekly-review.log"
            local total=$(wc -l < "$DATA_DIR/weekly-review.log")
            echo "  [Linkbox] weekly-review: $input"
    ...
    *)
        ...
        echo "Run 'linkbox help' for available commands."
        exit 1
        ;;
esac