#!/usr/bin/env python3
"""
Simmer Kalshi Weather Trading Skill

Trades Kalshi weather markets using NOAA forecasts via DFlow on Solana.
Kalshi port of the popular polymarket-weather-trader skill.
...
Requires:
    ...
    SOLANA_PRIVATE_KEY environment variable (base58-encoded)
    KYC verification at dflow.net/proof
"""
...
TRADE_SOURCE = "sdk:kalshi-weather"
SKILL_SLUG = "kalshi-weather-trader"
_automaton_reported = False

# Kalshi constraints
MIN_SHARES_PER_ORDER = 1.0  # Kalshi minimum is 1 contract
...
# Supported locations (matching Kalshi/NOAA resolution sources)
LOCATIONS = {
...
# =============================================================================
# Market Discovery - Auto-import from Kalshi
# =============================================================================
# Searches Simmer's importable endpoint (venue=kalshi) for weather markets.
# Kalshi weather tickers use the KXHIGH/KXLOW prefix (e.g., KXHIGHNY-26FEB19).
# Trading does NOT depend on discovery — trades from already-imported
# markets via GET /api/sdk/markets?tags=weather.
# =============================================================================

# Search terms per location (matching Kalshi event naming)
...
def discover_and_import_weather_markets(log=print):
    """Discover weather markets on Kalshi and auto-import to Simmer.
        ...
        for term in search_terms:
            try:
                results = client.list_importable_markets(
                    q=term, venue="kalshi", min_volume=1000, limit=20
                )
            except Exception as e:
            ...
            for m in results:
                ...
                # Filter: must be a temperature market on Kalshi
                if "temperature" not in question:
                    continue
                if "kalshi.com" not in url:
                    continue

                # Try to import via Kalshi import endpoint
                try:
...
def fetch_weather_markets():
    """Fetch Kalshi weather markets from Simmer API.

    Kalshi weather markets are tagged 'kalshi' (not 'weather'), so we filter
    by import_source and then check question text for temperature keywords.
    """
    try:
        result = get_client()._request("GET", "/api/sdk/markets",
                                       params={"status": "active", "import_source": "kalshi", "q": "temperature", "limit": 100})
...
def run_weather_strategy(dry_run: bool = True, positions_only: bool = False,
    ...
    log("\n🔍 Discovering new weather markets on Kalshi...")
