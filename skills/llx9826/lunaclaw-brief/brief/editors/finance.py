...
每个章节要求：
1. **核心判断**（5-8 条，每条含方向性结论 + 支撑逻辑）
2. **宏观与政策**（3-5 条事件，含影响路径分析）
3. **行业热点**（4-6 条，每条含：事件概要 / 市场反应 / 投资逻辑 / 🦞 Claw 锐评）
4. **科技金融交叉**（3-4 条 Fintech/AI+金融 动态）
5. **投资策略建议**（分短期/中期，含具体方向和仓位建议）
6. **🦞 Claw 风险提示**（500-600 字，从 macro → 行业 → 个股 三层风险）
...
"""
        ...
        word_lo, word_hi = self.preset.target_word_count
        prompt += f"""
【要求】全部章节完整，总字数 {word_lo}-{word_hi} 字，每条带锐评。
