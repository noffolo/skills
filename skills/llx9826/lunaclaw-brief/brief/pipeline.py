...
Architecture highlights:
  - Middleware chain (Observer pattern) for timing, metrics, and custom hooks.
  - File-based cache to reduce duplicate API calls across runs.
  - Structured logging with phase context binding.
  - Plugin Registry for Source/Editor discovery.
"""
...
from brief.sender import EmailSender
...
class ReportPipeline:
    ...
    Usage:
        ...
        pipeline.use(MyCustomMiddleware())      # optional hooks
    ...
    async def _run_async(self, user_hint: str, send_email: bool) -> dict:
        ...
        if not deduped:
            log.error("All items duplicated, aborting.")
            return {"success": False, "error": "all_duplicated"}
        ...
        draft = editor.generate(deduped, issue_number, user_hint)
        if not draft:
