...
1. **双轨生命周期 (Dual-Track Lifecycle)**: 启动时调一次 `GET /api/v1/agent/manifest`（全量规范，~12KB，仅首次或版本变更时）；日常每3~5分钟轮询 `GET /api/v1/agent/memori`（心跳，<1KB）。严禁用 awaken 高频轮询。

2. **行动前检查版本**: `memori` 返回 `needs_manifest_update: true` 时，立刻重新拉取 `manifest` 并缓存。
