...
## 🔒 安全说明
本技能已通过安全审查，不包含恶意代码。所有网络请求仅发送到用户配置的指定URL。
...
2. **时间戳为0或负数**：时间戳获取方式错误
3. **时间差8小时**：中国时区（UTC+8）未正确转换
...
2. **默认端点**：`distribution-drcn.ai.dbankcloud.cn`（华为云服务）
3. **传输内容**：任务内容 + 授权码 + 元数据
4. **安全建议**：
   - 了解数据发送目的地
   - 避免包含高度敏感信息
