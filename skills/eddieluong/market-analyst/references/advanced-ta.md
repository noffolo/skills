...
| **38.2%** | Hồi trung bình — trend mạnh | Vùng mua tốt nếu uptrend intact |
| **50.0%** | Hồi nửa đường — phổ biến nhất | Key level, nhiều trader chờ ở đây |
| **61.8%** | Golden Ratio — hồi sâu | Cơ hội mua tốt nhất nếu trend giữ |
...
| **138.2%** | Target moderate |
| **161.8%** | Target phổ biến nhất — sóng 3 thường kết thúc ở đây |
| **200%** | Double move |
| **261.8%** | Extended target (sóng 3 rất mạnh) |

**Cách dùng:**
1. Xác định sóng 1 (Wave 1): đáy A → đỉnh B
2. Xác định pullback: đỉnh B → đáy C
3. Fibonacci Extension từ A qua B đến C → target cho sóng 3
...
| Sóng | Đặc điểm | Fibonacci target |
|------|----------|-----------------|
| **Wave 1** | Bắt đầu trend mới, ít người nhận ra | — |
| **Wave 2** | Correction, không vượt quá 100% W1 | Retrace 50-61.8% của W1 |
| **Wave 3** | **Mạnh nhất, dài nhất** — KHÔNG bao giờ ngắn nhất | 161.8% - 261.8% của W1 |
...
| Sóng | Đặc điểm |
|------|----------|
| **Wave A** | Bắt đầu điều chỉnh, nhiều người nghĩ là pullback bình thường |
...
Wave 1: 650 → 1,500 (đỉnh 01/2022)
Wave 2: 1,500 → 870 (đáy 11/2022) — retrace 73% ✅ (deep nhưng < 100%)
Wave 3: 870 → 1,300 (đang hình thành?)
→ Nếu đúng, Wave 3 target: 161.8% × Wave 1 = 870 + 1.618 × 850 = ~2,245

Nhưng: VN-Index hiện tại 1,604, đang correction
→ Cần xác nhận Wave 3 đã kết thúc hay đang trong sub-wave correction
```
...
```
Giá xa POC → kỳ vọng quay về POC
Entry: Khi giá ở ngoài VA và có tín hiệu đảo chiều
Target: POC
Stop: Ngoài VA + buffer
```

#### Strategy 2: HVN Support/Resistance
```
Giá tiếp cận HVN từ trên → Expect support
Giá tiếp cận HVN từ dưới → Expect resistance
Breakout HVN với volume lớn → Trend continuation mạnh
```

#### Strategy 3: LVN Breakout
```
Giá break qua LVN → Move nhanh đến HVN tiếp theo
Entry: Khi giá vượt LVN
Target: HVN kế tiếp
...
**Ví dụ:**
MBB: Fibonacci 61.8% = 19,350 | HVN = 19,000-20,000 | EMA200 = 19,500
→ Vùng 19,000-20,000 là **mua cực mạnh** nếu giá về đến đây
...
→ Đang ở Wave nào? (1-5 impulse hay A-B-C corrective?)
→ Wave tiếp theo hướng nào?

Bước 2: FIBONACCI — Xác định target/support chính xác
→ Nếu đang trong correction: dùng Retracement để tìm support
→ Nếu đang trong impulse: dùng Extension để tìm target

Bước 3: VOLUME PROFILE — Xác nhận mức giá quan trọng
→ Fibonacci level có trùng HVN không? → Strong confirmation
→ POC ở đâu? Giá đang trên hay dưới POC?

Bước 4: HÀNH ĐỘNG
→ Confluence ≥ 3 signals → Mua/Bán mạnh
→ Confluence 2 signals → Mua/Bán vừa phải
...
```
Elliott Wave: Đang ở Wave 2 correction (từ đỉnh 28k → về 20k)
→ Wave 2 retrace ~57% (gần 61.8%) → gần kết thúc

Fibonacci: 61.8% retracement = 19,350
→ Hỗ trợ mạnh tại 19,000-20,000

Volume Profile: HVN lớn tại 20,000-21,000
→ Confirm vùng support

Kết luận: Vùng 19,000-21,000 = BUY ZONE MẠNH
Target Wave 3: 161.8% extension ≈ 32,000-35,000
Score nâng cao: +3 (Fib+HVN) + +3 (W3 setup) + +2 (POC) = +8
```
...
4. Label Elliott Wave nếu pattern rõ ràng
5. Tìm **confluence zones** (nơi nhiều levels trùng nhau)
...
- ⚠️ Chỉ nhìn lại (retrospective) thì rõ ràng, nhìn tới (prospective) thì khó
- ✅ Kết hợp với Fibonacci giúp giảm tính chủ quan

### Fibonacci
- ⚠️ **Self-fulfilling prophecy** — nhiều trader dùng nên levels có hiệu ứng
- ⚠️ Không phải magic — cần kết hợp với volume và trend
- ✅ Tốt nhất khi dùng cùng Volume Profile

### Volume Profile
- ⚠️ Cần data quality tốt (không phải platform nào cũng có)
- ⚠️ Quá khứ không đảm bảo tương lai
- ✅ Objective hơn Elliott Wave — dựa trên data thực
