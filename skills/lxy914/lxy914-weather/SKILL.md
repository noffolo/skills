...
```bash
# 查询实时天气
uv run scripts/qweather_cli.py now 北京

# 查询天气预报（默认 7 天，支持 3/7/10/15/30 天）
uv run scripts/qweather_cli.py daily 上海
uv run scripts/qweather_cli.py daily 太原 3

# 查询小时预报（默认 24 小时）
uv run scripts/qweather_cli.py hourly 广州
uv run scripts/qweather_cli.py hourly 深圳 12
```
