#!/usr/bin/env bash
# Feedback — content tool
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

DATA_DIR="${HOME}/.local/share/feedback"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }
_version() { echo "feedback v2.0.0"; }

_help() {
    echo "Feedback v2.0.0 — content toolkit"
    echo ""
    echo "Usage: feedback <command> [args]"
    echo ""
    echo "Commands:"
    echo "  draft              Draft"
    echo "  edit               Edit"
...
_search() {
    local term="${1:?Usage: feedback search <term>}"
...
case "${1:-help}" in
    draft)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/draft.log" 2>/dev/null || echo "  No entries yet. Use: feedback draft <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/draft.log"
            local total=$(wc -l < "$DATA_DIR/draft.log")
            echo "  [Feedback] draft: $input"
    ...
    edit)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/edit.log" 2>/dev/null || echo "  No entries yet. Use: feedback edit <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/edit.log"
            local total=$(wc -l < "$DATA_DIR/edit.log")
            echo "  [Feedback] edit: $input"
    ...
    optimize)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/optimize.log" 2>/dev/null || echo "  No entries yet. Use: feedback optimize <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/optimize.log"
            local total=$(wc -l < "$DATA_DIR/optimize.log")
            echo "  [Feedback] optimize: $input"
    ...
    schedule)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/schedule.log" 2>/dev/null || echo "  No entries yet. Use: feedback schedule <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/schedule.log"
            local total=$(wc -l < "$DATA_DIR/schedule.log")
            echo "  [Feedback] schedule: $input"
    ...
    hashtags)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/hashtags.log" 2>/dev/null || echo "  No entries yet. Use: feedback hashtags <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/hashtags.log"
            local total=$(wc -l < "$DATA_DIR/hashtags.log")
            echo "  [Feedback] hashtags: $input"
    ...
    hooks)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/hooks.log" 2>/dev/null || echo "  No entries yet. Use: feedback hooks <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/hooks.log"
            local total=$(wc -l < "$DATA_DIR/hooks.log")
            echo "  [Feedback] hooks: $input"
    ...
    cta)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/cta.log" 2>/dev/null || echo "  No entries yet. Use: feedback cta <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/cta.log"
            local total=$(wc -l < "$DATA_DIR/cta.log")
            echo "  [Feedback] cta: $input"
    ...
    rewrite)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/rewrite.log" 2>/dev/null || echo "  No entries yet. Use: feedback rewrite <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/rewrite.log"
            local total=$(wc -l < "$DATA_DIR/rewrite.log")
            echo "  [Feedback] rewrite: $input"
    ...
    translate)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/translate.log" 2>/dev/null || echo "  No entries yet. Use: feedback translate <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/translate.log"
            local total=$(wc -l < "$DATA_DIR/translate.log")
            echo "  [Feedback] translate: $input"
    ...
    tone)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/tone.log" 2>/dev/null || echo "  No entries yet. Use: feedback tone <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/tone.log"
            local total=$(wc -l < "$DATA_DIR/tone.log")
            echo "  [Feedback] tone: $input"
    ...
    headline)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/headline.log" 2>/dev/null || echo "  No entries yet. Use: feedback headline <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/headline.log"
            local total=$(wc -l < "$DATA_DIR/headline.log")
            echo "  [Feedback] headline: $input"
    ...
    outline)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/outline.log" 2>/dev/null || echo "  No entries yet. Use: feedback outline <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/outline.log"
            local total=$(wc -l < "$DATA_DIR/outline.log")
            echo "  [Feedback] outline: $input"
    ...
    *)
        echo "Unknown: $1 — run 'feedback help'"
        exit 1
        ;;
esac