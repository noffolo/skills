#!/usr/bin/env bash
# guided-learning-cn - Chinese content creation tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${GUIDED_LEARNING_CN_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/guided-learning-cn}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
guided-learning-cn v$VERSION

Chinese content creation tool

Usage: guided-learning-cn <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "guided-learning-cn v$VERSION" ;;
