...
DATA_DIR="${HOME}/.local/share/fuel"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }
_version() { echo "fuel v2.0.0"; }

_help() {
    echo "Fuel v2.0.0 — home toolkit"
    echo ""
    echo "Usage: fuel <command> [args]"
    echo ""
    echo "Commands:"
    echo "  add                Add"
...
_search() {
    local term="${1:?Usage: fuel search <term>}"
...
case "${1:-help}" in
    add)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/add.log" 2>/dev/null || echo "  No entries yet. Use: fuel add <input>"
        else
            local input="$*"
    ...
    inventory)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/inventory.log" 2>/dev/null || echo "  No entries yet. Use: fuel inventory <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/inventory.log"
            local total=$(wc -l < "$DATA_DIR/inventory.log")
            echo "  [Fuel] inventory: $input"
    ...
    schedule)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/schedule.log" 2>/dev/null || echo "  No entries yet. Use: fuel schedule <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/schedule.log"
            local total=$(wc -l < "$DATA_DIR/schedule.log")
            echo "  [Fuel] schedule: $input"
    ...
    remind)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/remind.log" 2>/dev/null || echo "  No entries yet. Use: fuel remind <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/remind.log"
            local total=$(wc -l < "$DATA_DIR/remind.log")
            echo "  [Fuel] remind: $input"
    ...
    checklist)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/checklist.log" 2>/dev/null || echo "  No entries yet. Use: fuel checklist <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/checklist.log"
            local total=$(wc -l < "$DATA_DIR/checklist.log")
            echo "  [Fuel] checklist: $input"
    ...
    usage)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/usage.log" 2>/dev/null || echo "  No entries yet. Use: fuel usage <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/usage.log"
            local total=$(wc -l < "$DATA_DIR/usage.log")
            echo "  [Fuel] usage: $input"
    ...
    cost)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/cost.log" 2>/dev/null || echo "  No entries yet. Use: fuel cost <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/cost.log"
            local total=$(wc -l < "$DATA_DIR/cost.log")
            echo "  [Fuel] cost: $input"
    ...
    maintain)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/maintain.log" 2>/dev/null || echo "  No entries yet. Use: fuel maintain <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/maintain.log"
            local total=$(wc -l < "$DATA_DIR/maintain.log")
            echo "  [Fuel] maintain: $input"
    ...
    log)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/log.log" 2>/dev/null || echo "  No entries yet. Use: fuel log <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
    ...
    report)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/report.log" 2>/dev/null || echo "  No entries yet. Use: fuel report <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/report.log"
            local total=$(wc -l < "$DATA_DIR/report.log")
            echo "  [Fuel] report: $input"
    ...
    seasonal)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/seasonal.log" 2>/dev/null || echo "  No entries yet. Use: fuel seasonal <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/seasonal.log"
            local total=$(wc -l < "$DATA_DIR/seasonal.log")
            echo "  [Fuel] seasonal: $input"
    ...
    tips)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/tips.log" 2>/dev/null || echo "  No entries yet. Use: fuel tips <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/tips.log"
            local total=$(wc -l < "$DATA_DIR/tips.log")
            echo "  [Fuel] tips: $input"
