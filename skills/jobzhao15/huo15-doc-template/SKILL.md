...
```python
from huo15_doc import create_formatted_doc

# 一条命令搞定：页面设置 + 页眉LOGO + 页脚页码 + 字体规范
...
def get_company_info():
    ...
    # 1. 尝试从本地配置文件读取
    config_dir = os.path.dirname(COMPANY_CONFIG_PATH)
    if os.path.exists(COMPANY_CONFIG_PATH):
        try:
            with open(COMPANY_CONFIG_PATH, 'r', encoding='utf-8') as f:
                config = json.load(f)
                company_name = config.get('company_name', DEFAULT_COMPANY_NAME)
    ...
    # 优先尝试公司系统 LOGO
    for url in [COMPANY_LOGO_URL, FALLBACK_LOGO_URL]:
        try:
...
def ensure_logo_exists(logo_path=DEFAULT_LOGO_PATH):
    ...
    try:
        ...
        print(f"✓ LOGO 已下载: {logo_path}")
    except Exception as e:
        raise FileNotFoundError(f"❌ LOGO 下载失败: {e}\n请手动下载 LOGO 放到: {logo_path}")
...
def add_header_with_logo(doc, logo_path=None, company_name=None):
    ...
    # 确保 LOGO 存在
    logo_path = ensure_logo_exists(logo_path)
