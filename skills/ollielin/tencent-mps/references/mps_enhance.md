...
**功能**：视频画质增强、老片修复、超分辨率、音频分离/人声提取/伴奏提取、HDR、插帧等。
...
- **音频分离必须先确认类型**：用户请求音频分离/提取音轨/人声提取时，若未明确说明类型（人声/伴奏/背景声），**必须先追问，严禁猜测或使用默认值**："请问您需要提取哪种音轨？人声（vocal）、伴奏（accompaniment）还是背景声（background）？" 即使用户只说"提取音轨"也不得假设为 `vocal`，必须追问后再生成命令。
