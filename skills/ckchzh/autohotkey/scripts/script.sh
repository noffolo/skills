#!/usr/bin/env bash
# autohotkey - Multi-purpose utility tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${AUTOHOTKEY_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/autohotkey}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
autohotkey v$VERSION

Multi-purpose utility tool

Usage: autohotkey <command> [args]
...
case "${1:-help}" in
    ...
    info) shift; cmd_info "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "autohotkey v$VERSION" ;;
