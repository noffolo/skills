...
DATA_DIR="${HOME}/.local/share/mailcheck"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }
_version() { echo "mailcheck v2.0.0"; }

_help() {
    echo "Mailcheck v2.0.0 — ai toolkit"
    echo ""
    echo "Usage: mailcheck <command> [args]"
    echo ""
    echo "Commands:"
...
_search() {
    local term="${1:?Usage: mailcheck search <term>}"
...
case "${1:-help}" in
    configure)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/configure.log" 2>/dev/null || echo "  No entries yet. Use: mailcheck configure <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/configure.log"
            local total=$(wc -l < "$DATA_DIR/configure.log")
            echo "  [Mailcheck] configure: $input"
    ...
    benchmark)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/benchmark.log" 2>/dev/null || echo "  No entries yet. Use: mailcheck benchmark <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/benchmark.log"
            local total=$(wc -l < "$DATA_DIR/benchmark.log")
            echo "  [Mailcheck] benchmark: $input"
    ...
    compare)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/compare.log" 2>/dev/null || echo "  No entries yet. Use: mailcheck compare <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/compare.log"
            local total=$(wc -l < "$DATA_DIR/compare.log")
            echo "  [Mailcheck] compare: $input"
    ...
    prompt)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/prompt.log" 2>/dev/null || echo "  No entries yet. Use: mailcheck prompt <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/prompt.log"
            local total=$(wc -l < "$DATA_DIR/prompt.log")
            echo "  [Mailcheck] prompt: $input"
    ...
    evaluate)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/evaluate.log" 2>/dev/null || echo "  No entries yet. Use: mailcheck evaluate <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/evaluate.log"
            local total=$(wc -l < "$DATA_DIR/evaluate.log")
            echo "  [Mailcheck] evaluate: $input"
    ...
    fine-tune)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/fine-tune.log" 2>/dev/null || echo "  No entries yet. Use: mailcheck fine-tune <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/fine-tune.log"
            local total=$(wc -l < "$DATA_DIR/fine-tune.log")
            echo "  [Mailcheck] fine-tune: $input"
    ...
    analyze)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/analyze.log" 2>/dev/null || echo "  No entries yet. Use: mailcheck analyze <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/analyze.log"
            local total=$(wc -l < "$DATA_DIR/analyze.log")
            echo "  [Mailcheck] analyze: $input"
    ...
    cost)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/cost.log" 2>/dev/null || echo "  No entries yet. Use: mailcheck cost <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/cost.log"
            local total=$(wc -l < "$DATA_DIR/cost.log")
            echo "  [Mailcheck] cost: $input"
    ...
    usage)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/usage.log" 2>/dev/null || echo "  No entries yet. Use: mailcheck usage <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/usage.log"
            local total=$(wc -l < "$DATA_DIR/usage.log")
            echo "  [Mailcheck] usage: $input"
    ...
    optimize)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/optimize.log" 2>/dev/null || echo "  No entries yet. Use: mailcheck optimize <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/optimize.log"
            local total=$(wc -l < "$DATA_DIR/optimize.log")
            echo "  [Mailcheck] optimize: $input"
    ...
    test)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/test.log" 2>/dev/null || echo "  No entries yet. Use: mailcheck test <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/test.log"
            local total=$(wc -l < "$DATA_DIR/test.log")
            echo "  [Mailcheck] test: $input"
    ...
    report)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/report.log" 2>/dev/null || echo "  No entries yet. Use: mailcheck report <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/report.log"
            local total=$(wc -l < "$DATA_DIR/report.log")
            echo "  [Mailcheck] report: $input"
    ...
    *)
        echo "Unknown: $1 — run 'mailcheck help'"
        exit 1
        ;;
esac