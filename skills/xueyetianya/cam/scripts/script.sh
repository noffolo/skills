#!/usr/bin/env bash
# cam -- Computer-aided manufacturing tool
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${CAM_DIR:-$HOME/.cam}"
...
show_help() {
    cat << EOF
cam v$VERSION -- Computer-aided manufacturing tool

Usage: cam <command> [args]
...
cmd_add() {
    local value="${1:?Usage: cam add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: cam search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: cam remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "cam v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: cam help"; exit 1 ;;
esac
